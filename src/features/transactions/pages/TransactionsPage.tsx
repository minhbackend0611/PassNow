...
function TransactionItem({
  ...
  return (
    <div id={`tx-${tx.id}`} className={`glass-panel bg-surface-container-lowest/80 backdrop-blur-xl border border-white/60 rounded-[32px] p-6 md:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-500 hover:-translate-y-1 flex flex-col gap-6 group ${displayStatus === 'cancelled' ? 'opacity-75 grayscale-[0.2]' : ''}`}>
      ...
      {/* Visual Stepper */}
      <div className="relative grid grid-cols-2 w-[85%] max-w-3xl mx-auto mt-4 mb-4">
      ...
      <div className="flex flex-row items-stretch justify-end gap-2 pt-3 w-full border-t border-outline-variant/20 mt-2">
        ...
        {tx.status === 'pending' && !isBuyer && !tx.sellerConfirmed && (
          ...
          >
            <span className="material-symbols-outlined text-[20px]">{listing && (listing.completedCount || 0) >= (listing.quantity || 1) ? 'block' : 'local_shipping'}</span>
            <span className="whitespace-normal sm:whitespace-nowrap">{listing && (listing.completedCount || 0) >= (listing.quantity || 1) ? 'Sold Out' : 'Confirm Delivery'}</span>
          </button>
        )}
